...
class Cli {
  ...
  generateTests(testOptions, name) {
    ...
    if (testOptions) {
      switch (testOptions.toLowerCase()) {
        case 'i':
          ejs.renderFile(
            path.join(__dirname, './templates/Tests/IntTest.ejs'),
            data,
            (err, str) => {
              fs.promises
                .mkdir(
  ...
  generateModel(name) {
    ...
    ejs.renderFile(
      path.join(__dirname, settings.databaseHandler !== 'mongoose' ? './templates/Model.ejs' : './templates/MongooseModel.ejs'),
      data,
      (err, str) => {
        fs.promises
          .mkdir(`${path.join(__dirname, settings.modelDir)}/`, {
            recursive: true,
          })
          .then(() => {
            fs.writeFile(
